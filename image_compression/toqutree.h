...
public:
   ...
   * This function will call helper function buildTree.
   ...
   /* Helper function */
   PNG render(Node *, PNG &, int, int);
   
   pair<int,int> find_split(int size, stats cur_stats, int k);
	
   void makePNG(PNG & og, PNG & other, pair<int,int> ul, int size);
   
   HSLAPixel render_find(Node * root, int x_s, int y_s);

   pair<int,int> rel_coord(int spl_x, int spl_y, int size, int cx, int cy, int node);
